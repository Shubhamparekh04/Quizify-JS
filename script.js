...
for (let i = 0; i < 4; i++) {
    labelOpts[i].innerHTML = radioOpt[i].value = `${quizQuestions[0].options[i]}`;
}
...
for (let i = 0; i < radioOpt.length; i++) {
    radioOpt[i].addEventListener('click', () => {
        radioOpt[i].value == quizQuestions[index].answer ? correctAns++ : correctAns--;
    });
}
...
const updateOptions = () => {
    ...
    quizQuestions[index].options.forEach((opt, i) => {
        labelOpts[i].innerHTML = radioOpt[i].value = opt;
    });
};
updateOptions();
...
function endQuiz() {
    container.innerHTML = `<div class="text-center"><h3>Quiz Over</h3><p>Your Score: <strong>${correctAns} / ${quizQuestions.length}</strong></p><button class="btn btn-primary" onclick="location.reload()">Restart</button></div>`;
